...
import {ISvcDexPair} from "./interfaces/ISvcDexPair.sol";
import {ISvcDexRouter02} from "./interfaces/ISvcDexRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by SvcDexSwap teams.
 */
contract SvcDexZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // SvcDexRouter interface
    ISvcDexRouter02 public svcDexRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from SvcDexSwap)
    uint256 public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint256 public maxZapReverseRatio;

    // Address SvcDexRouter
    address private svcDexRouterAddress;
     ...
     * @param _svcDexRouter: address of the SvcDexRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WBNBAddress,
        address _svcDexRouter,
        uint256 _maxZapReverseRatio
    ) {
        WBNBAddress = _WBNBAddress;
        WBNB = IWETH(_WBNBAddress);
        svcDexRouterAddress = _svcDexRouter;
        svcDexRouter = ISvcDexRouter02(_svcDexRouter);
    ...
    {
        address token0 = ISvcDexPair(_lpToken).token0();
        address token1 = ISvcDexPair(_lpToken).token1();
        ...
        (uint256 reserveA, uint256 reserveB, ) = ISvcDexPair(_lpToken)
            .getReserves();
        ...
        if (token0 == _tokenToZap) {
            ...
            swapAmountOut = svcDexRouter.getAmountOut(
                swapAmountIn,
                reserveA,
                reserveB
            );
    ...
    {
        require(
            _token0ToZap == ISvcDexPair(_lpToken).token0() ||
                _token0ToZap == ISvcDexPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == ISvcDexPair(_lpToken).token0() ||
                _token1ToZap == ISvcDexPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == ISvcDexPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            ...
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) /
                ISvcDexPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = svcDexRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = svcDexRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = ISvcDexPair(_lpToken).burn(
            address(this)
        );
        ...
        // Swap tokens
        svcDexRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (
            IERC20(_token).allowance(address(this), svcDexRouterAddress) < 1e24
        ) {
            // Re-approve
            IERC20(_token).safeApprove(svcDexRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = svcDexRouter.getAmountOut(
            halfToken0Amount,
            _reserve0,
            _reserve1
        );
        uint256 denominator = svcDexRouter.quote(
            halfToken0Amount,
